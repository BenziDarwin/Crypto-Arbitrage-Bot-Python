...
TRADING_CONFIG = {
    "borrow_token": "USDT",      # Token to flash loan
...
"bsc_testnet": {
        ...
        "tokens": {
            ...
            "USDT": "0x1ae0b71d22d52920dc8e6990b092b5ebdec0db8c",
        },
    },
}
...
class ArbitrageBot:
    ...
    def _configure_testnet_routers(self, opportunity: Dict) -> bool:
        ...
        try:
            ...
            # Buy router: USDT -> WBNB
            wbnb_bought = (BORROW_AMOUNT / buy_price) * (1 - buy_fee)
            buy_output_wei = int(wbnb_bought * 10**18)
            
            # Sell router: WBNB -> USDT
            busd_received = wbnb_bought * sell_price * (1 - sell_fee)
            sell_output_wei = int(busd_received * 10**18)
            
            log(f"     Buy on {buy_router_name}: {BORROW_AMOUNT} USDT → {wbnb_bought:.6f} WBNB", Colors.CYAN)
            log(f"     Sell on {sell_router_name}: {wbnb_bought:.6f} WBNB → {busd_received:.6f} USDT", Colors.CYAN)
    ...
    def find_arbitrage_opportunity(self) -> Dict:
        ...
        # Use 1 WBNB to get price per WBNB in USDT
        wbnb_amount = 10**18  # 1 WBNB
        path_wbnb_to_busd = [token_intermediate, token_borrow]  # WBNB -> USDT
        ...
        for router_name in router_names:
            ...
            # Get how much USDT for 1 WBNB (this is the price)
            busd_for_wbnb = self.get_mainnet_price(router, wbnb_amount, path_wbnb_to_busd)
            if busd_for_wbnb:
                # Convert to float price (USDT per WBNB)
    ...
    def execute_arbitrage_v2(self, opportunity: Dict) -> Optional[str]:
        ...
        try:
            ...
            tx = self.arbitrage_contract.functions.executeArbitrageV2(
                ...
                True,                                                 # isBase (USDT is base token)
